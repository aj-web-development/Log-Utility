...
/**
 * One labeled log output (e.g. "Application", "Error", "Access") written by a {@link LogSource}
 * node. Carries the live log-file path, the rotated-backup root and its naming pattern (using
 * {@code {date}/{HH}/{i}} placeholders), plus the cached result of the last path-availability check.
 */
@Entity
@Table(name = "log_file")
@Getter
...
public class LogFile {
    ...
    /** Optional — only needed to disambiguate multiple outputs on the same node. */
    @Column(name = "file_label", length = 200)
